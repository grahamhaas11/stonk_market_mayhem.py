...
def buy_stock():
    ...
    If the user has sufficient funds, deducts the total cost from available cash and updates the portfolio with the purchased shares. Prints a confirmation message on success or an error message if funds are insufficient.
    """
    global money
...
def sell_stock():
    ...
    Prompts the user to enter a stock symbol and the number of shares to sell. If the user owns enough shares, adds the proceeds to available cash and reduces the portfolio holdings. Prints a confirmation or an error message if the user does not own enough shares.
    """
    global money
...
def view_portfolio():
    ...
    Prints each stock in the portfolio with the number of shares owned, followed by the remaining cash.
    """
